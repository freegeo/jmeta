...
test_behaviour() ->
  {error, {wrong_namespace, 'some.strange.namespace'}} = ?MODULE:get({'some.strange.namespace', 'some.undefined.meta'}),
  {error, {{std, 'some.undefined.meta'}, is_not_defined}} = ?MODULE:get({std, 'some.undefined.meta'}).
